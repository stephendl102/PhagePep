...
def createFasta(PepFile, filenameoutput):
    tableref1 = pd.read_excel(PepFile,index_col=0,header=0).reset_index()
    print(tableref1)
    list_name = tableref1.iloc[0:1999,1].tolist()
    list_seq = tableref1.iloc[0:1999,1].tolist()
    print(list_seq)
...
for i in range(len(snakemake.input)):
    AddCharge(snakemake.input[i],snakemake.output[i])
