...
filename = "./Nonspecifics/NonspecificsScoresCharacteristics.xlsx"
posnum = 2
libnum = 2
...
def Correlation(filename):
    print('File is: ' + str(filename))
    d = pd.read_excel(filename,index_col=0,header=0).reset_index()
    ...
    print(np.mean(y))
    print(y)
    X = np.absolute(df.iloc[:,4:].values)
    ...
    absCharge = np.absolute(df['Charge'].values)
    Frequency = df['Score'].values
    print(scipy.stats.spearmanr(absCharge, Frequency))
    df.plot.scatter(x='Charge', y='Score', c='DarkBlue')
    plt.show()
