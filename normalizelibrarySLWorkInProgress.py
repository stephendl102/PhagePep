...
def normalizelibrarySL(library,reflibrary,libraryname,average_negative=False):
    ...
    mergelibrary.dropna(axis=0,inplace=True) #keep only those that overlap with reference library
    print(mergelibrary)

    Seqs = mergelibrary['Peptide'].reset_index(drop=True) # array of strings (sequences)
    Nums = mergelibrary.drop('Peptide','columns').reset_index(drop=True)
    print(Nums)
